            ...
            if (sp1.IsOpen)     //此处可能没有必要判断是否打开串口，但为了严谨性，我还是加上了
            {
                ...
                else                                            //'发送16进制按钮'
                {
                    try
                    {
                        ...
                        if (strRcv.IndexOf("ID") != -1)
                        {
                            string[] sArray = strRcv.Split(':');
                            txtcardID.Text = sArray[1];
                        }
                        if (strRcv.IndexOf("Read") != -1)
                        {
                            string[] sArray = strRcv.Split(':');
                            Int32 money = (sArray[1][1]-1)*256*256 + (sArray[1][2]-1)*256 + (sArray[1][3]-1);
                            ...
                            while (mqdata.Read())
                            {
                                if (mqdata.GetString(1) != null)
                                {
                                    ...
                                    if ( ta== "")
                                    {
                                        ...
                                        comm = new MySQLCommand("UPDATE rfid_car SET date = " + time + " WHERE ID = '" + txtcardID.Text + "'", mq);
                                        comm.ExecuteNonQuery();
                                    ...
                                    else
                                    {
                                        ...
                                        ts = DateTime.Now - Convert.ToDateTime(mqdata.GetString(4));
                                        string flag = "9usetime:" + ts.TotalSeconds.ToString() + "in:" + mqdata.GetString(4) + "out:" + DateTime.Now.ToString() + "\r\n";
                                        comm = new MySQLCommand("UPDATE rfid_car SET date = NULL WHERE ID = '" + txtcardID.Text + "'", mq);
                                        comm.ExecuteNonQuery();
                                        sp1.WriteLine(flag);
                                    }
...
//                byteBuffer =byteBuffer+ "\r\n";
            ...
            {
                string sendbuf = txtSend.Text + "\r\n";
                sp1.WriteLine(sendbuf);    //写入数据
            }
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (sp1.IsOpen)
            {
                txtID = txtcardID.Text;
                txtmoney = txtMoney.Text;
...
//            txtmoney = txtMoney.Text;
            Form1 form = new Form1();
            form.ShowDialog();
            string mon = Form1.txt1money;
//            txtMoney.Text = mon;
//            char t = Convert.ToChar(mon);
            //mon = '3' + t + "\r\n";
            sp1.WriteLine("3"+mon+"\r\n"); 
        }

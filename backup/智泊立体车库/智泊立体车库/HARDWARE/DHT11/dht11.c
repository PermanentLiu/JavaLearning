...
{   
    ...
    while (DHT12_DQ_IN&&retry<100)//DHT12会拉低40~80us
	{
		retry++;
		delay_us(1);
	};	 
    ...
    while (!DHT12_DQ_IN&&retry<100)//DHT12拉低后会再次拉高40~80us
	{
		retry++;
		delay_us(1);
	};
...
{	 
 	...
 	GPIO_SetBits(GPIOG,GPIO_Pin_10);						 //PG11 输出高
